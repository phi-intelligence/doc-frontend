...
import { Image as ImageIcon, ArrowLeft, Loader2, Download, Save, Upload, X } from 'lucide-react';
...
const ProductStagingPage = () => {
  ...
  // Chat pipeline state
  const [sessionId] = useState(() => `product-staging-${Date.now()}`);
  ...
  const handleProcess = async () => {
    ...
    const message = `Place the product from "${uploadedFile.filename}" into an AI-generated environment.
Environment description: ${backgroundDescription || 'Modern, clean studio setting with soft lighting'}
...
Create a professional product staging image suitable for marketing.`;
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `product-staging-result-${Date.now()}.png`;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            {processCard && (processCard.status === 'processing' || processCard.status === 'error') && (
              <ProcessCard
                title="Product Staging Processing"
          ...
          <div className={`rounded-xl border p-6 ${
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className={`text-lg font-semibold flex items-center gap-2 ${
                ...
                <ImageIcon className={`w-5 h-5 ${isDark ? 'text-brand-accent-400' : 'text-brand-accent-500'}`} />
                Result Preview
              </h3>
              {resultImage && (
...
export default ProductStagingPage;
