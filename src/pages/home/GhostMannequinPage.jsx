...
import { Package, ArrowLeft, Loader2, Download, Save, Upload, X } from 'lucide-react';
...
const GhostMannequinPage = () => {
  ...
  // Chat pipeline state
  const [sessionId] = useState(() => `ghost-mannequin-${Date.now()}`);
  ...
  const handleProcess = async () => {
    ...
    const message = `Process the clothing image "${uploadedFile.filename}" using ghost mannequin technique.
...
Fill in any gaps left by mannequin removal naturally.`;
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `ghost-mannequin-result-${Date.now()}.png`;
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            <Package className={`w-8 h-8 ${isDark ? 'text-brand-accent-400' : 'text-brand-accent-500'}`} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            {processCard && (processCard.status === 'processing' || processCard.status === 'error') && (
              <ProcessCard
                title="Ghost Mannequin Processing"
          ...
          <div className={`rounded-xl border p-6 ${
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className={`text-lg font-semibold flex items-center gap-2 ${
                ...
                <Package className={`w-5 h-5 ${isDark ? 'text-brand-accent-400' : 'text-brand-accent-500'}`} />
                Result Preview
              </h3>
              {resultImage && (
              ...
              <div className={`aspect-square rounded-xl border-2 border-dashed flex items-center justify-center ${
                ...
                <div className="text-center p-8">
                  ...
                  <p className={`text-sm ${
                    ...
                    Upload a clothing image and click Process to see results
                  </p>
                </div>
              </div>
            )}
