...
import HRPromptSuggestions from '../components/hr/HRPromptSuggestions';
import Header from '../components/layout/Header';

/**
 * HRIntegrationPage - Specialized page for HR document generation
 * Three-column layout: FileUploadSidebar | DocumentViewer | AIChatSidebar
 * Complete feature parity with ChatPage
 */
const HRIntegrationPage = () => {
  // Session management
  ...
  // HR-specific prompt suggestions
  const hrSuggestions = [
    "Generate all onboarding documents for this candidate",
    "Create complete HR package: offer letter, contract, NDA, and ID card",
    "Generate offer letter and employment contract",
    "Create employee ID card from uploaded photo",
  ...
  const handleSendMessage = useCallback(async (message, contextFiles, signal) => {
    ...
    try {
      ...
      const assistantMessage = {
        role: 'assistant',
        content: response.response || 'Documents generated successfully',
        timestamp: new Date().toISOString(),
        artifacts: response.new_artifacts || response.artifacts || []
      };
      ...
      if (error.name !== 'AbortError') {
        ...
        const errorMessage = {
          role: 'assistant',
          content: `Error: ${error.message || 'Failed to generate documents'}`,
          timestamp: new Date().toISOString(),
          isError: true
        };
...
export default HRIntegrationPage;
