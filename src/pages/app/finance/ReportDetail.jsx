  ...
  const fetchDetail = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data?.detail || 'Failed to load report detail');
      }
  ...
  return (
    <div className="max-w-6xl">
      <div className="flex items-center justify-between gap-4">
        <Link to="/app/finance/reports" className="inline-flex items-center gap-2 text-sm font-semibold text-brand-accent-600 hover:text-brand-accent-700">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-light-text tracking-tight">
              {item?.title || `Report ${id}`}
            </h1>
            <p className="mt-1 text-sm text-light-text-secondary">
              Status: <span className="font-bold">{(item?.status || 'unknown').toUpperCase()}</span>
            </p>
          </div>
          <Link to="/demo/finance" className="btn-primary px-4 py-2 rounded-xl">
          ...
          <div className="card p-4">
            ...
            <div className="space-y-2">
              ...
              {(!item?.artifacts || item.artifacts.length === 0) ? (
                <div className="text-sm text-light-text-secondary">No generated artifacts yet.</div>
              ) : null}
            </div>
          </div>
        </div>
