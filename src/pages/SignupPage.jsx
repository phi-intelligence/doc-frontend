...
import { Mail, User, Sparkles, Github, Chrome } from 'lucide-react';
import { useTheme } from '../context/ThemeContext';

/**
 * Signup Page - Email/OTP and Social Auth
...
const SignupPage = () => {
  ...
  const handleOTPSubmit = (e) => {
    e.preventDefault();
    // UI only - no implementation
    console.log('OTP signup:', { email: formData.email, otp: formData.otp });
  };
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center px-4 sm:px-6 lg:px-8 py-8 sm:py-12 ${isDark ? 'bg-dark-bg' : 'bg-light-bg'} transition-colors duration-300`}>
        ...
        >
          ...
          <div className="text-center mb-8">
            ...
            <p className={`text-sm ${isDark ? 'text-dark-text-secondary' : 'text-light-text-secondary'}`}>
              Start your journey with Phi Docs
            </p>
          </div>
            ...
            >
              <div>
                ...
                <div className="relative">
                  <User className={`absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 ${isDark ? 'text-dark-text-muted' : 'text-light-text-muted'}`} />
                  <input
                    type="text"
                    value={formData.name}
              ...
              <div className="flex items-start gap-2">
                <input
                  ...
                  className={`mt-0.5 w-4 h-4 rounded border ${isDark ? 'border-dark-border bg-dark-bg' : 'border-light-border bg-white'} text-brand-accent-500 focus:ring-brand-accent-500`}
                  required
                />
                <label htmlFor="terms" className={`text-sm cursor-pointer ${isDark ? 'text-dark-text-secondary' : 'text-light-text-secondary'}`}>
                  I agree to the{' '}
                  <a href="#" className={`font-medium hover:underline ${isDark ? 'text-brand-accent-400' : 'text-brand-accent-600'}`}>
                    Terms of Service
                  </a>
                  {' '}and{' '}
                  <a href="#" className={`font-medium hover:underline ${isDark ? 'text-brand-accent-400' : 'text-brand-accent-600'}`}>
              ...
              <button
                ...
                className={`w-full py-3 px-4 rounded-lg font-semibold text-white transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${isDark ? 'bg-brand-accent-600 hover:bg-brand-accent-500' : 'bg-brand-accent-500 hover:bg-brand-accent-600'} shadow-lg hover:shadow-xl`}
              >
                Send OTP
              </button>
            </motion.form>
            ...
            >
              <div>
                <p className={`text-sm mb-4 ${isDark ? 'text-dark-text-secondary' : 'text-light-text-secondary'}`}>
                  We've sent a 6-digit code to <span className={`font-medium ${isDark ? 'text-dark-text' : 'text-light-text'}`}>{formData.email}</span>
                </p>
                <label className={`block text-sm font-medium mb-2 ${isDark ? 'text-dark-text' : 'text-light-text'}`}>
                  Enter OTP
                </label>
              ...
              <div className="flex gap-3">
                <button
                  type="button"
                  onClick={() => { setOtpSent(false); updateFormData('otp', ''); }}
          ...
          <div className={`mt-6 text-center text-sm ${isDark ? 'text-dark-text-secondary' : 'text-light-text-secondary'}`}>
            ...
            <Link to="/login" className={`font-semibold hover:underline ${isDark ? 'text-brand-accent-400' : 'text-brand-accent-600'}`}>
              Sign in
            </Link>
          </div>
        </motion.div>
