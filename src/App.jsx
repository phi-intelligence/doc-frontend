...
function App() {
  ...
  const extractSkillFromEvent = (text) => {
    if (!text) return null;
    // Try to extract skill name from patterns like:
    // "Generating javascript code for docx..."
    // "Generating python code for xlsx..."
    const match = text.match(/for\s+(\w+)/i);
    if (match && match[1]) {
      return match[1].toLowerCase();
    }
    // Fallback: try to find skill name directly in text
  ...
  useEffect(() => {
    if (!isProcessing) {
      // Clear pending artifact when not processing
      if (pendingArtifact) {
    ...
    // Look for code_start events in progress stream
    ...
    if (codeStartEvent) {
      ...
      if (skillName && (!pendingArtifact || !pendingArtifact.filename.includes(skillName))) {
        ...
        // Add to allFiles if not already there
        setAllFiles(prev => {
  ...
  return (
    <div className="flex h-screen w-full bg-light-bg">
      ...
      <div className={`flex flex-col h-full border-r border-light-border transition-all duration-300 ${allFiles.length > 0 ? (sidebarCollapsed ? 'w-[38%]' : 'w-[34%]') : 'flex-1'}`}>
        ...
        {(processCards.length === 0 && uploadedFiles.length === 0) ? (
          <WelcomeScreen
            onSend={(text) => handleSendMessage({ preventDefault: () => { } }, text)}
          ...
          <>
            ...
            <div className="border-t border-light-border bg-light-sidebar p-4">
              ...
              <form onSubmit={handleSendMessage} className="flex items-center gap-3 bg-light-bg border border-light-border rounded-xl px-3 py-2 focus-within:border-light-border-hover transition-colors">
                ...
                <button
                  type="button"
                  className="p-2 text-light-text-secondary hover:text-light-text hover:bg-white rounded-lg transition-all"
                ...
                <div className="hidden md:flex items-center gap-1 text-xs text-light-text-secondary px-2">
                  <span>Phi Docs</span>
