...
const WelcomeScreen = ({
  ...
  return (
    <div className={`flex-1 flex flex-col items-center justify-start p-4 md:p-6 overflow-y-auto ${isDark ? 'bg-dark-bg' : 'bg-light-bg'}`}>
      <motion.div
      ...
      >
        ...
        <div className="mb-6 text-center">
          ...
          <h1 className={`text-2xl md:text-3xl font-display font-semibold mb-2 ${isDark ? 'text-dark-text' : 'text-light-text'}`}>
            {skill ? (
              <span className={`text-transparent bg-clip-text bg-gradient-to-r ${isDark ? 'from-brand-accent-500 to-brand-accent-400' : 'from-brand-accent-400 to-brand-accent-600'}`}>{info.name}</span>
            ) : (
              <>Hi, I'm <span className={`text-transparent bg-clip-text bg-gradient-to-r ${isDark ? 'from-brand-accent-500 to-brand-accent-400' : 'from-brand-accent-400 to-brand-accent-600'}`}>Phi Docs</span></>
            )}
          </h1>
          <p className={`text-sm max-w-md mx-auto ${isDark ? 'text-dark-text-secondary' : 'text-light-text-secondary'}`}>
            {info.subtitle}
          </p>
        </div>
        ...
        <div className="w-full max-w-2xl relative">
          <form onSubmit={handleSubmit} className={`flex items-center gap-2 border rounded-lg px-2 py-2 transition-colors ${isDark ? 'bg-dark-sidebar border-dark-border focus-within:border-dark-border-hover' : 'bg-light-sidebar border-light-border focus-within:border-light-border-hover'}`}>
            <button
              type="button"
              onClick={() => fileInputRef.current?.click()}
              disabled={isUploading}
              className={`p-1.5 rounded transition-all disabled:opacity-50 disabled:cursor-not-allowed ${isDark ? 'text-brand-accent-400 hover:text-brand-accent-300 hover:bg-brand-accent-900/30' : 'text-brand-accent-500 hover:text-brand-accent-600 hover:bg-brand-accent-50'}`}
              title="Attach files"
            >
              {isUploading ? (
            ...
            <input
              ...
              placeholder="Reply..."
              className={`flex-1 py-1.5 bg-transparent border-none outline-none text-sm ${isDark ? 'text-dark-text placeholder-dark-text-muted' : 'text-light-text placeholder-light-text-muted'}`}
              autoFocus
            />
            <button
              type="submit"
              disabled={!val.trim() && !selectedTemplate}
              className={`p-2 text-white rounded transition-all disabled:opacity-40 disabled:cursor-not-allowed ${isDark ? 'bg-brand-accent-600 hover:bg-brand-accent-500' : 'bg-brand-accent-500 hover:bg-brand-accent-600'}`}
            >
          ...
          {selectedTemplate && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              className={`mt-2 flex items-center gap-2 px-3 py-2 border rounded-lg text-sm ${isDark ? 'bg-brand-accent-900/30 border-brand-accent-800 text-brand-accent-300' : 'bg-brand-accent-50 border-brand-accent-200 text-brand-accent-700'}`}
            >
              <FileText className="w-4 h-4" />
              <span>Using template: <strong>{selectedTemplate.name}</strong></span>
              <button
                onClick={() => setSelectedTemplate(null)}
                className={`ml-auto font-bold ${isDark ? 'text-brand-accent-400 hover:text-brand-accent-300' : 'text-brand-accent-600 hover:text-brand-accent-800'}`}
        ...
        <div className="mt-6 w-full max-w-2xl">
          <p className={`text-[11px] font-medium uppercase mb-3 text-center ${isDark ? 'text-dark-text-muted' : 'text-light-text-muted'}`}>Or try one of these</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {suggestions.map((item, idx) => (
              <button
                key={idx}
                onClick={() => onSend(item.prompt)}
                className={`flex flex-col items-center justify-center p-3 border rounded-lg transition-all group ${isDark ? 'bg-dark-surface border-dark-border hover:border-brand-accent-600' : 'bg-white border-light-border hover:border-brand-accent-300'}`}
              >
                <div className={`p-2 rounded-lg mb-2 transition-colors ${isDark ? 'bg-brand-accent-900/30 group-hover:bg-brand-accent-800/50' : 'bg-brand-accent-50 group-hover:bg-brand-accent-100'}`}>
                  <item.icon className={`w-4 h-4 ${isDark ? 'text-brand-accent-400' : 'text-brand-accent-500'}`} />
                </div>
                <span className={`text-[12px] font-medium ${isDark ? 'text-dark-text' : 'text-light-text'}`}>{item.label}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        {showTemplates && (
          <motion.div
            ...
            className={`mt-8 w-full border-t pt-8 ${isDark ? 'border-dark-border' : 'border-light-border'}`}
          >
            {/* Section Header */}
            <div className="mb-6">
              <h2 className={`text-lg font-semibold mb-1 ${isDark ? 'text-dark-text' : 'text-light-text'}`}>Templates</h2>
            ...
            {/* Category Tabs */}
            <div className={`flex gap-2 mb-6 border-b overflow-x-auto ${isDark ? 'border-dark-border' : 'border-light-border'}`}>
              {TEMPLATE_CATEGORIES.map(category => (
                <button
                  ...
                  className={`px-4 py-2 text-sm font-medium transition-colors whitespace-nowrap border-b-2 ${selectedCategory === category.id
                    ? isDark ? 'text-brand-accent-400 border-brand-accent-500' : 'text-brand-accent-600 border-brand-accent-600'
                    : isDark ? 'text-dark-text-secondary border-transparent hover:text-dark-text hover:border-dark-border' : 'text-light-text-secondary border-transparent hover:text-light-text hover:border-light-border'
                    }`}
                >
                  {category.label}
                </button>
            ...
            {loadingTemplates ? (
              <div className="flex items-center justify-center py-12">
                <RefreshCw className={`w-6 h-6 animate-spin ${isDark ? 'text-brand-accent-400' : 'text-brand-accent-500'}`} />
                <span className={`ml-3 ${isDark ? 'text-dark-text-secondary' : 'text-light-text-secondary'}`}>Loading templates...</span>
              </div>
            ) : templates.length > 0 ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                ...
                <button
                  onClick={() => setSelectedTemplate(null)}
                  className={`flex flex-col items-center justify-center p-4 border-2 border-dashed rounded-xl transition-all group ${isDark ? 'bg-dark-surface border-dark-border hover:border-brand-accent-600' : 'bg-white border-light-border hover:border-brand-accent-300'}`}
                >
                  <div className={`w-full aspect-[3/4] rounded-lg flex items-center justify-center mb-2 ${isDark ? 'bg-dark-bg' : 'bg-light-bg'}`}>
                    <FileText className={`w-12 h-12 transition-colors ${isDark ? 'text-dark-text-secondary group-hover:text-brand-accent-400' : 'text-light-text-secondary group-hover:text-brand-accent-500'}`} />
                  </div>
                  <p className={`text-sm font-medium ${isDark ? 'text-dark-text' : 'text-light-text'}`}>Blank Document</p>
                  <p className={`text-xs ${isDark ? 'text-dark-text-secondary' : 'text-light-text-secondary'}`}>Start from scratch</p>
                </button>
              </div>
            ) : (
